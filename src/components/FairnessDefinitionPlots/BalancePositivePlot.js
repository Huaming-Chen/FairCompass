...
class BalancePositivePlot extends Component {
    constructor(props) {
        super(props);
    ...
    loadData = () => {
        ...
        for (var s of this.props.activeGroups) {
            ...
            temp.push({
                name: name,
                pppscore: s.metrics.pppscore,
                size: s.insts.length
            })
        }
        temp = temp.sort((a,b) => a.pppscore - b.pppscore); // b - a for reverse sort
    ...
    render () {        
        return(
            <div>
                <h3>
                Average Positive Predicted Probability Score Chart
                </h3>
                <ResponsiveContainer width="95%" height={400}>
                    <BarChart data={this.state.data}>
                        ...
                        <Tooltip />
                        <Legend />
                        <Bar dataKey="pppscore" fill="#8884d8" />
                    </BarChart>
                </ResponsiveContainer>
            </div>
...
export default BalancePositivePlot;
